...
module Operations =
    ...
    let dotnet (args: string) =
        Cmd.createWithArgs "dotnet" (args.Split(' ') |> List.ofArray)
        |> Cmd.run
...
module Steps =
    let packNugets =
        Step.create "Pack Nugets" {
            ...
            Console.info "Generating NuGet Package"
    ...
    let zip =
        Step.create "Zip binaries" {
            ...
            Console.info $"Binaries Zipped at '{outDir}'"
            |> ctx.Console.WriteLine
        }
...
module Pipelines =
    ...
    let buildRuntime =
        Pipeline.createFrom packNuget "build:runtime" {
