...
type DevServerArgs =
    ...
    interface IArgParserTemplate with
        member this.Usage =
            match this with
            | Server _ -> "Starts a development server for modern Javascript development"
...
let main argv =
    ...
    try
        let results =
            parser.ParseCommandLine(inputs = argv, raiseOnUsage = true, ignoreMissing = true, ignoreUnrecognized = true)
