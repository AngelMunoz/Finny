...
type TestRunnerArgs =
  ...
  interface IArgParserTemplate with
    member this.Usage =
      ...
      | Root_Directory _ ->
        "Sets The directory from where to load sources and tests alike."
  ...
  static member ToOptions(args: ParseResults<TestRunnerArgs>) =
    ...
    let browsers =
      ...
      | Some true -> [| Chromium; Chrome; Edge; Firefox; Webkit |]
      | _ ->
        let res = args.TryGetResult(Browsers) |> Option.map Array.ofSeq

        match res with
        | Some [||] -> [| Chromium |]
        | Some value ->
          value |> Set.ofArray |> Set.map Browser.FromBrowserEnum |> Set.toArray
        | _ -> [| Chromium |]

    let serveDir = args.TryGetResult(Root_Directory) |> Option.flatten

    let testsDirectory =
      match args.TryGetResult(Tests_Directory) |> Option.flatten with
...
module Commands =
  ...
  let private getOnlyTheseScripts rootDir testDir (names: string array) =
    ...
    |> Array.filter (fun path ->
      names |> Array.exists (fun name -> path.Contains(name)))

  let getScriptsOnlyInTheseDirectories
    rootDir
    testDir
    ...
    seq {
      for directory in directories do
        let path = Path.Combine(rootDir, testDir, directory)
  ...
  let private startTestRun (getApp: unit -> WebApplication) (browser: Browser) =
    let getBrowser
      browser
      (browserOptions: BrowserTypeLaunchOptions)
      (pl: IPlaywright)
      =
      match browser with
    ...
    task {
      ...
      opts.Predicate <-
        fun console ->
          console.Text = "Nacre:Finished:true" && console.Type = "info"
  ...
  let runTests (opts: TestRunnerOptions) =
    ...
    let suite =
      ...
      | ExecutionSuite (scripts, groups) ->
        let scripts = getOnlyTheseScripts rootDir testsDir scripts

        let groups = getScriptsOnlyInTheseDirectories rootDir testsDir groups
