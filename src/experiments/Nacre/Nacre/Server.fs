...
module TestLogger =
  ...
  let private logError (error: TestError) =
    ...
    printfn
      $"{error.name}: [Actual: {error.actual} - Expected: {error.expected}]"
...
module Server =
  ...
  let private clientHandler
    (suite: string array * Map<string, string array>)
    _
    (ctx: HttpContext)
    =
    task {
      let path = Path.Combine(Path.NacreExecPath, "test.tpl.html")
  ...
  let private socketScriptHandler _ (ctx: HttpContext) =
    task {
      ...
      let path = Path.Combine(Path.NacreExecPath, "mock.js")
  ...
  let private clientMessageHandler _ (ctx: HttpContext) =
    task {
      let! msg =
        JsonSerializer.DeserializeAsync<SendMessagePayload>(ctx.Request.Body)

      TestLogger.LogMessage msg
      return! ctx.WriteJsonAsync({|  |})
    }

  let private getWebApp
    (suite: string array * Map<string, string array>)
    : HttpHandler =
    choose
      [ GET
        >=> choose
              [ route "/" >=> (clientHandler suite)
                route "/__web-dev-server__web-socket.js" >=> socketScriptHandler ]
        POST >=> route "/~nacre~/messages" >=> clientMessageHandler ]
