...
task {
  let tasks =
    [ for file in filesToProcess do
        asyncOption {
          let! plugin =
            Extensibility.LoadPluginFromScript(
              "xml-to-json",
              Path.GetFullPath "./Plugin.fsx"
            )
  ...
  for file in files do
    ...
    | (Some file, _) ->
      printfn
        $"Output: {file.mimeType}, {UTF8.GetString(file.content).Substring(0, 100)}..."
    | (None, args) ->
      printfn
        $"We were not able to process this file %A{args.source |> Path.GetFileName}"
}
|> Async.AwaitTask
