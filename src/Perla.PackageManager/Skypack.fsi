...
type Skypack =
    ...
    static member PackageUrls:
        name: string * ?minified: bool * ?esVersion: string * ?typescriptTypes: bool -> Task<ImportUrls>

    /// Makes an http call to the skypack api to search possible package candidates for the given name
    static member SearchPackage: name: string * ?page: int -> Task<PackageSearchResults>
